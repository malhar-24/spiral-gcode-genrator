...
def generate_spiral(num_turns, spacing, num_points, scale_factor, degree, feed_rate, delay_start, delay_end, inner_radius):
...
def index():
    if request.method == 'POST':
        ...
        delay_end = float(request.form['delay_end'])
        inner_radius = float(request.form['inner_radius'])  # New parameter

        # Generate spiral and G-code
        svg_file, gcode_file = generate_spiral(num_turns, spacing, num_points, scale_factor, degree, feed_rate, delay_start, delay_end, inner_radius)
